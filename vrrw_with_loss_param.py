...
from gnn_with_loss_param import load_trained_gnn, load_trained_prediction
...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='ba2motifs', choices=['mutagenicity', 'aids', 'nci1', 'proteins'])
...
def counterfactual_summary_with_randomwalk(input_graphs, importance_args, teleport_probability, max_steps, loss_penalty):
    ...
    torch.save(save_item, f'results/{dataset_name}/runs/with_loss_penalty/counterfactuals_{loss_penalty}.pt')
...
if __name__ == '__main__':
    ...
    gnn_model = load_trained_gnn(dataset_name, args.loss_penalty, device=device1)
    gnn_model.eval()

    # Load prediction based on model
    preds = load_trained_prediction(dataset_name, args.loss_penalty, device=device1)
    ...
    counterfactual_summary_with_randomwalk(input_graphs=input_graphs,
                                           importance_args=importance_args,
                                           teleport_probability=teleport_probability,
                                           max_steps=max_steps, loss_penalty=args.loss_penalty)
